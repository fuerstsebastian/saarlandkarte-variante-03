 ...
 */let o=[],c=[],l=null;const u={zeitstellung:"all",kategorie_attraktion:"all",kategorie_befund:"all",sichtbarkeit:"all",barrierefreiheit:"all"};async function w(){console.log("Initializing Archaeological App of Saarland..."),o=await k(),c=[...o],l=new y("map-element",B),l.init(),l.updateMarkers(o),E(),b(o),$(),g(o.length,o.length)}function E(){const s=document.getElementById("filter-befund"),e=document.getElementById("filter-kategorie_attraktion");s&&Array.from(new Set(o.map(a=>a.kategorie_befund).filter(Boolean))).sort().forEach(a=>{const n=document.createElement("option");n.value=a,n.textContent=a,s.appendChild(n)}),e&&Array.from(new Set(o.map(a=>a.kategorie_attraktion).filter(Boolean))).sort().forEach(a=>{const n=document.createElement("option");n.value=a,n.textContent=a,e.appendChild(n)})}function b(s){const e=document.getElementById("locations-grid");if(e){if(s.length===0){e.innerHTML=`
